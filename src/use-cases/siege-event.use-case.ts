...
export class SiegeEventUseCase {
  ...
  async handleJobSelection(
    ...
    action: 'added' | 'removed' | 'moved_to_candidate' | 'promoted_from_candidate';
    fromRole?: string;
  }> {
    ...
    const currentPrincipals = (updatedPrincipals.get(jobClass) as unknown as string[]) || [];
    const currentCandidates = (updatedCandidates.get(jobClass) as unknown as string[]) || [];
    ...
    for (const [role, userIds] of updatedPrincipals) {
      ...
      if (ids.includes(userId)) {
        ...
        const candidates = (updatedCandidates.get(role) as unknown as string[]) || [];
